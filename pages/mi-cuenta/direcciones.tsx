...
import {
  direccionSchema,
  DireccionSchemaType,
} from "@/lib/mi-cuenta/direccionSchema";
import { FaPencil, FaTrash } from "react-icons/fa6";
import ProtectedRoute from "@/components/ProtectedRoute";
import { apiGet, apiPost, apiPut, apiDelete } from "@/lib/auth/apiClient";
...
export default function Direcciones() {
  ...
  const [direccionEditando, setDireccionEditando] = useState<string | null>(
    null,
  );
  ...
  const [errors, setErrors] = useState<
    Partial<Record<keyof DireccionSchemaType, string>>
  >({});
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (
        ...
        direccionPrincipal &&
        direccionPrincipal.id !== direccionEditando
      ) {
        ...
        await apiPut(
          `/users/addresses/${direccionPrincipal.id}`,
          prevPrincipalData,
        );
      }

      const url = direccionEditando
        ? `/users/addresses/${direccionEditando}`
        : `/users/addresses`;

      let response;
      if (direccionEditando) {
        response = await apiPut(url, addressData);
      } else {
        response = await apiPost(url, addressData);
      }
      ...
      if (saveResult.success) {
        showToast(
          `Dirección ${direccionEditando ? "actualizada" : "guardada"} correctamente`,
        );
                        ...
                        <div className="mb-6">
                          ...
                          <div className="bg-white rounded-sm shadow-sm p-6 border-2 border-primary/20">
                            <div className="flex justify-between items-start mb-3">
                              <h3 className="font-semibold text-primary-dark text-lg">
                                {direccionPrincipal.apartment
                                  ? `Dirección principal: ${direccionPrincipal.address}`
                                  : "Dirección principal"}
                              </h3>
                              <div className="flex gap-2">
                                <button
                                  onClick={() =>
                                    editarDireccion(direccionPrincipal)
                                  }
                                ...
                                <button
                                  onClick={() =>
                                    eliminarDireccion(direccionPrincipal.id)
                                  }
                            ...
                            <p className="text-gray-600 text-sm mb-1">
                              {direccionPrincipal.district},{" "}
                              {direccionPrincipal.province},{" "}
                              {direccionPrincipal.department}
                            </p>
                            ...
                            <p className="text-gray-600 text-sm">
                              <span className="font-semibold">Dpto/Piso:</span>{" "}
                              {direccionPrincipal.apartment}
                            </p>
                          </div>
                        </div>
