...
export default function StoresModal({
    ...
    useEffect(() => {
        ...
        const loadStores = async () => {
            ...
            try {
                ...
                const location = currentLocations.find(l => l.desDistrito === selectedLocation);
    ...
    const modalContent = isOpen && (
        <div className={`fixed inset-0 z-[9999] flex items-center justify-center p-4 ${outfit.variable} font-sans`}>
            {/* Overlay */}
            <div
            ...
            <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-3xl max-h-[90vh] overflow-hidden flex flex-col">
                ...
                <div className="px-6 pb-4 pt-10 flex items-center justify-center flex-shrink-0">
                    ...
                    <div className="text-center">
                        <h2 className="text-2xl md:text-3xl font-semibold text-primary-dark">Nuestras Tiendas</h2>
                        <p className="text-gray-500 mt-2">Encuentra tu punto de recojo más cercano</p>
                    </div>
                </div>
                    ...
                    <div className="space-y-3">
                        {loadingLocations ? (
                            <div className="text-center py-8">
                                ...
                                <p className="mt-2 text-gray-500 text-sm">Cargando ubicaciones...</p>
                            </div>
                            ...
                            currentLocations.map((location) => (
                                <div key={location.codUbigeoAlm} className="border-2 border-gray-200 rounded-sm overflow-hidden">
                                    {/* Header del dropdown */}
                                    <button
                                        onClick={() => handleLocationToggle(location.desDistrito)}
                                        className={`w-full px-4 md:px-6 py-2 md:py-4 flex items-center justify-between transition-colors ${selectedLocation === location.desDistrito
                                            ? 'bg-green-50'
                                    ...
                                    >
                                        <div className="flex items-center gap-3">
                                            ...
                                            <span className="font-semibold text-gray-900 text-left text-sm md:text-base">{location.desDistrito}</span>
                                        </div>
                                        <svg
                                            className={`w-5 h-5 text-gray-400 transition-transform ${selectedLocation === location.desDistrito ? 'rotate-180' : ''
                                                }`}
                                            fill="none"
                                            stroke="currentColor"
                                            viewBox="0 0 24 24"
                                        ...
                                        <div className="bg-white border-t-2 border-gray-200">
                                            {loadingStores ? (
                                                ...
                                                mapWarehouses.map((store) => {
                                                    const details = warehouseDetails.find(d => d.idAlmacen === store.idAlmacen);
                                                    return (
                                                        <div key={store.idAlmacen} className="px-6 py-4 hover:bg-gray-50 transition-colors border-b last:border-b-0 border-gray-100">
                                                            <h3 className="font-semibold text-gray-900 mb-1">{store.desAlmacen}</h3>
                                                            {details && (
                                                                <div className="space-y-1">
                                                                    <p className="text-gray-600 text-sm"><span className="font-medium">Dirección:</span> {details.direccion}</p>
                                                                    <p className="text-gray-600 text-xs"><span className="font-medium">Horario:</span> {details.atencion}</p>
                                                                </div>
                                                            )}
                                                        </div>
                                                    );
