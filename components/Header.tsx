...
import { HiMenuAlt2, HiChevronRight } from 'react-icons/hi';
import { FaHeart, FaUser, FaShoppingCart, FaTruck, FaBoxes, FaSearch } from 'react-icons/fa';
...
const topLinks = [
  ...
  { href: '/favoritos', label: 'Mis favoritos', icon: <FaHeart size={12} /> },
...
function Logo({ width = 100, height = 40, className = '' }) {
  return (
    <Link href="/" className={`flex items-center ${className}`}>
      <Image src={logo} alt="Liwilu Logo" width={width} height={height} priority />
    </Link>
  );
}
...
function SearchBar({ isMobile = false }) {
  return (
    ...
    >
      ...
      <button
        className={`${
          isMobile ? 'ml-2 hover:text-green-700' : ''
...
function QuickActions({ isMobile = false, cartCount = 0 }) {
    ...
    return (
      <div className="flex items-center gap-4">
        ...
        <Link href="/carrito" className="relative">
          ...
          {cartCount > 0 && (
            <span className="absolute -top-2 -right-2 bg-green-400 text-[#0b2d2d] font-bold text-[10px] w-5 h-5 rounded-full flex items-center justify-center">
              {cartCount}
            </span>
          )}
        </Link>
  ...
  return (
    <div className="flex items-center gap-6 text-sm">
      <Link href="/mayorista" className="flex items-center gap-2 hover:text-green-400 transition">
        <FaBoxes /> Compra mayorista
      </Link>
      <Link href="/pedido" className="flex items-center gap-2 hover:text-green-400 transition">
        <FaTruck /> Sigue tu pedido
      </Link>
      <Link href="/cuenta" className="flex items-center gap-2 hover:text-green-400 transition">
        <FaUser /> Mi cuenta
      </Link>
      <Link href="/carrito" className="relative hover:text-green-400 transition">
        <FaShoppingCart size={18} />
        {cartCount > 0 && (
...
export default function Header() {
  ...
  return (
    ...
    >
      {/* 🔹 TOP BAR */}
      <div
        className={`bg-green-400 text-[12px] lg:text-xs py-1 px-4 transition-all duration-300 ${
          ...
          <div className="lg:hidden space-y-3">
            ...
            <div className="flex items-center gap-3">
              <button
                ...
                className="flex items-center gap-2 p-2 bg-white/10 hover:bg-white/15 rounded-md transition"
              >
                <HiMenuAlt2 className="text-[22px]" />
                      ...
                      <li key={c.href}>
                        <Link
                          ...
                          className={`block px-4 py-3 text-white transition-colors ${
                            c.highlight
                              ? 'bg-green-400 hover:bg-green-500 rounded-xl font-medium text-[#0b2d2d]'
                              : c.highlightBottom
                      ...
                      {categories.map((c) => (
                        <li key={c.href}>
                          ...
                          >
                            ...
                            <HiChevronRight
                              className={c.highlight ? 'text-white' : 'text-gray-400'}
                            />
                          </Link>
                        </li>
                      ))}
